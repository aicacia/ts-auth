...
<script lang="ts">
	...
	import type { Application, ApplicationPagination } from '$lib/openapi/auth';
	...
	async function loadMore() {
		...
		let pagniation: ApplicationPagination;
		loading = true;
		try {
			pagniation = await applicationApi.allApplications(page * DEFAULT_SIZE, DEFAULT_SIZE);
		} finally {
			loading = false;
		}
